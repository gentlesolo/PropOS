    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-1 space-y-5">
            ...
            <div class="glass-panel rounded-2xl border border-border-default/60 p-5">
                ...
                <!-- Update Status -->
                <div class="pt-4 border-t border-border-default/60 space-y-3">
                    <select wire:model.defer="newStatus" class="w-full rounded-xl border border-border-default bg-surface-input px-3 py-2 text-sm text-text-primary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary">
                        @foreach(['initiated', 'fica_pending', 'fica_verified', 'offer_accepted', 'conveyancing', 'registration', 'completed', 'cancelled'] as $s)
                        <option value="{{ $s }}">{{ ucwords(str_replace('_', ' ', $s)) }}</option>
                        @endforeach
                    </select>
                    <button wire:click="updateStatus" class="w-full py-2 bg-brand-primary text-white rounded-xl text-sm font-medium hover:bg-brand-secondary transition-colors">
            ...
            <div class="glass-panel rounded-2xl border border-border-default/60 p-5">
                ...
                <form wire:submit.prevent="saveDeadlines" class="space-y-3">
                    <div>
                        ...
                        <input wire:model.defer="deadline" type="date" class="w-full rounded-lg border border-border-default bg-surface-input px-3 py-2 text-sm text-text-primary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-text-secondary mb-1">Est. Close Date</label>
                        <input wire:model.defer="estimated_close_date" type="date" class="w-full rounded-lg border border-border-default bg-surface-input px-3 py-2 text-sm text-text-primary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary">
                    </div>
                    <div>
                    ...
                    <button type="submit" class="w-full py-2 bg-brand-primary text-white rounded-xl text-sm font-medium hover:bg-brand-secondary transition-colors">Save</button>
                </form>
                @else
        ...
        <div class="xl:col-span-2 space-y-5">
            ...
            <div class="glass-panel rounded-2xl border border-border-default/60 p-5">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button wire:click="$toggle('showDocForm')" class="px-3 py-1.5 bg-brand-primary text-white rounded-lg text-xs font-medium hover:bg-brand-secondary transition-colors">
                ...
                <form wire:submit.prevent="uploadDocument" class="space-y-3 mb-5 p-4 bg-surface-sunken/30 rounded-xl border border-border-default/40">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select wire:model.defer="doc_type" class="w-full rounded-lg border border-border-default bg-surface-input px-3 py-2 text-sm text-text-primary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary">
                        ...
                        <div>
                            ...
                            <input wire:model.defer="doc_title" type="text" placeholder="Document title" class="w-full rounded-lg border border-border-default bg-surface-input px-3 py-2 text-sm text-text-primary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary">
                            @error('doc_title') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input wire:model.defer="doc_is_fica" type="checkbox" class="h-4 w-4 rounded border-border-default text-brand-primary focus:ring-brand-primary">

    ...
    <div class="bg-surface-card rounded-2xl border border-border-default p-5 mb-6">
        ...
        <form wire:submit.prevent="createInspection" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input wire:model="scheduled_at" type="datetime-local" class="w-full rounded-lg border border-border-default bg-surface-input px-3 py-2 text-sm text-text-primary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary focus:ring-offset-2 focus:ring-offset-surface-page">
            ...
            <div>
                ...
                <input wire:model="inspector_name" type="text" class="w-full rounded-lg border border-border-default bg-surface-input px-3 py-2 text-sm text-text-primary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary focus:ring-offset-2 focus:ring-offset-surface-page">
            </div>
            <div>
                <label class="block text-xs font-medium text-text-secondary mb-1">Inspector Company</label>
                <input wire:model="inspector_company" type="text" class="w-full rounded-lg border border-border-default bg-surface-input px-3 py-2 text-sm text-text-primary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary focus:ring-offset-2 focus:ring-offset-surface-page">
            </div>
            <div>
                <label class="block text-xs font-medium text-text-secondary mb-1">Estimated Cost ({{ $currencySymbol }})</label>
                <input wire:model="cost" type="number" min="0" class="w-full rounded-lg border border-border-default bg-surface-input px-3 py-2 text-sm text-text-primary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary focus:ring-offset-2 focus:ring-offset-surface-page">
            </div>
            <div class="md:col-span-2 flex gap-3 pt-2">
                ...
                <span wire:loading.remove>Schedule</span>
            ...
            <tbody class="divide-y divide-border-default">
                ...
                <tr class="hover:bg-surface-hover/30 transition-colors">
                    ...
                    <td class="px-4 py-3">
                        ...
                        <div class="flex gap-1.5">
                            <button wire:click="markComplete({{ $inspection->id }}, 'pass')" class="disabled:opacity-70 disabled:cursor-not-allowed relative text-xs px-2 py-0.5 bg-success-50 text-success-700 border border-success-200 rounded-md hover:bg-success-100 transition-colors" wire:loading.attr="disabled" wire:target="markComplete">
                <span wire:loading.remove wire:target="markComplete">Pass</span>
                <span wire:loading wire:target="markComplete" class="flex items-center space-x-2 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    <svg class="animate-spin h-4 w-4 text-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                            ...
                            <button wire:click="markComplete({{ $inspection->id }}, 'fail')" class="disabled:opacity-70 disabled:cursor-not-allowed relative text-xs px-2 py-0.5 bg-danger-50 text-danger-700 border border-danger-200 rounded-md hover:bg-danger-100 transition-colors" wire:loading.attr="disabled" wire:target="markComplete">
                <span wire:loading.remove wire:target="markComplete">Fail</span>
                <span wire:loading wire:target="markComplete" class="flex items-center space-x-2 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    <svg class="animate-spin h-4 w-4 text-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                </span>
            </button>

            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-zinc-900 border border-zinc-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full glassmorphism">
                    <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4 relative">
                        <button wire:click="close" class="disabled:opacity-70 disabled:cursor-not-allowed relative absolute top-4 right-4 text-zinc-400 hover:text-white" wire:loading.attr="disabled" wire:target="close">
                <span wire:loading.remove wire:target="close"><svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <span wire:loading wire:target="close" class="flex items-center space-x-2 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    ...
                    <div class="bg-zinc-800/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-zinc-800">
                        ...
                        <button type="button" wire:click="close" class="disabled:opacity-70 disabled:cursor-not-allowed relative mt-3 w-full inline-flex justify-center rounded-md border border-zinc-700 shadow-sm px-4 py-2 bg-zinc-800 text-base font-medium text-zinc-300 hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 focus:ring-offset-zinc-900 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" wire:loading.attr="disabled" wire:target="close">
                <span wire:loading.remove wire:target="close">Cancel</span>
                <span wire:loading wire:target="close" class="flex items-center space-x-2 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    <svg class="animate-spin h-4 w-4 text-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                </span>
            </button>

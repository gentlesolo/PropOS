<div>
    ...
    <button wire:click="toggle" class="fixed bottom-6 right-6 z-40 h-14 w-14 rounded-full bg-brand-primary text-white shadow-lg shadow-brand flex items-center justify-center hover:bg-brand-secondary transition-all hover:-translate-y-1 hover:shadow-xl focus:outline-none">
        ...
        <span class="absolute top-0 right-0 h-3.5 w-3.5 rounded-full bg-emerald-500 border-2 border-white dark:border-slate-900 animate-pulse"></span>
    </button>
    ...
    <div x-data="{ open: @entangle('isOpen') }" x-show="open" class="fixed inset-0 z-50 overflow-hidden" aria-labelledby="slide-over-title" role="dialog" aria-modal="true" style="display: none;">
        <div class="absolute inset-0 overflow-hidden">
            <!-- Background backdrop -->
            <div x-show="open" x-transition:enter="ease-in-out duration-500" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in-out duration-500" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" @click="open = false"></div>
                        ...
                        <div class="px-6 py-4 bg-brand-primary/5 border-b border-border-default/60 flex items-center justify-between">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs font-semibold text-emerald-500 flex items-center">
                                        <span class="h-1.5 w-1.5 rounded-full bg-emerald-500 mr-1.5 animate-pulse"></span> Online
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center space-x-3">

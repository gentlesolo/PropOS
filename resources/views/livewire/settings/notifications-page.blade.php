        ...
        <div class="bg-surface-card rounded-2xl border border-border-default p-6">
            ...
            <div class="space-y-3 {{ $leaseRemindersEnabled ? '' : 'opacity-40 pointer-events-none' }}">
                ...
                <div x-data="{ open: false }" class="rounded-xl border border-border-default bg-surface-hover/30 overflow-hidden">
                    ...
                    <div class="flex items-center gap-3 p-3">
                        ...
                        <button wire:click="removeReminder({{ $i }})"
                            class="disabled:opacity-70 disabled:cursor-not-allowed relative flex-shrink-0 text-text-tertiary hover:text-danger-600 transition-colors p-1 rounded-lg hover:bg-danger-50" wire:loading.attr="disabled" wire:target="removeReminder">
                <span wire:loading.remove wire:target="removeReminder"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg></span>
                <span wire:loading wire:target="removeReminder" class="flex items-center space-x-2 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                ...
                <button wire:click="addReminder"
                    class="disabled:opacity-70 disabled:cursor-not-allowed relative w-full py-2 border border-dashed border-border-default rounded-xl text-sm text-text-secondary hover:text-brand-primary hover:border-brand-primary transition-colors flex items-center justify-center gap-1.5" wire:loading.attr="disabled" wire:target="addReminder">
                <span wire:loading.remove wire:target="addReminder"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                    </svg>
                    Add Reminder</span>
                <span wire:loading wire:target="addReminder" class="flex items-center space-x-2 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    <svg class="animate-spin h-4 w-4 text-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                </span>
            </button>
                @endif

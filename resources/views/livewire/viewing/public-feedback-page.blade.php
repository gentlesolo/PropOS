        ...
        <div>
            ...
            <div class="flex gap-3">
                ...
                <button wire:click="$set('overall_rating', {{ $val }})"
                        class="flex-1 py-3 rounded-xl text-2xl border-2 transition
                               {{ $overall_rating == $val ? 'border-blue-500 bg-blue-50 scale-110' : 'border-gray-200 hover:border-gray-300' }}">
                    {{ $emoji }}
                </button>
                @endforeach
            </div>
        ...
        <div>
            ...
            <div class="flex gap-2">
                ...
                <button wire:click="$set('price_perception', {{ $val }})"
                        class="flex-1 py-2 rounded-lg text-xs font-medium border-2 transition
                               {{ $price_perception == $val ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600 hover:border-gray-300' }}">
                    {{ $label }}
                </button>
                @endforeach
            </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-2">
                ...
                <button wire:click="$set('interest_level', '{{ $val }}')"
                        class="py-2.5 rounded-xl text-sm font-medium border-2 transition
                               {{ $interest_level === $val ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600 hover:border-gray-300' }}">
                    {{ $label }}
                </button>
                @endforeach
            </div>

<div>
    ...
    <!-- Filters -->
    <div class="glass-panel rounded-2xl border border-border-default/60 overflow-hidden shadow-sm mb-2">
        <div class="px-6 py-4 border-b border-border-default/60 flex items-center justify-between bg-surface-sunken/30 gap-4">
            ...
            <div class="flex gap-2">
                ...
                <button wire:click="$set('filterStatus', '{{ $val }}')"
                    ...
                    {{ $filterStatus === $val ? 'bg-brand-primary text-white' : 'bg-surface-card border border-border-default/60 text-text-secondary hover:bg-surface-sunken' }}">
                    {{ $label }}
                </button>
                @endforeach
            </div>
        ...
        <div class="p-6">
            ...
            <div class="mb-4 p-5 bg-surface-card rounded-2xl border border-border-default/60 hover:shadow-md transition-shadow">
                ...
                <div class="flex items-center justify-between pt-3 border-t border-border-default/40">
                    ...
                    <div class="flex items-center gap-2">
                        <select wire:change="updateStatus({{ $campaign->id }}, $event.target.value)"
                            class="text-xs border border-border-default rounded-lg px-2 py-1 bg-surface-input text-text-primary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary">
            ...
            <div class="py-16 text-center">
                ...
                <a href="{{ route('marketing.campaign.new') }}" class="mt-3 inline-block px-4 py-2 bg-brand-primary text-white rounded-xl text-sm font-medium hover:bg-brand-secondary transition-colors">

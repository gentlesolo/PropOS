<div>
    ...
    <div class="flex gap-1 mb-6 bg-surface-hover/40 rounded-xl p-1 w-fit flex-wrap">
        ...
        <button wire:click="$set('activeReport','{{ $report }}')" class="disabled:opacity-70 disabled:cursor-not-allowed relative px-4 py-2 rounded-lg text-sm font-medium transition-colors {{ $activeReport === $report ? 'bg-surface-card text-text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary' }}" wire:loading.attr="disabled" wire:target="$set">

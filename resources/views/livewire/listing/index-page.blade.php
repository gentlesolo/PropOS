<div class="space-y-6 text-text-primary">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-surface-card/80 backdrop-blur-md p-4 rounded-xl border border-border-default/60 shadow-brand hover:border-brand-primary/40 transition-all duration-300">
            <p class="text-xs font-medium text-text-tertiary uppercase tracking-wider">Active Assets</p>
            <p class="mt-2 text-2xl font-bold text-text-primary font-mono tracking-tight">{{ $activeCount }}</p>
        </div>
        <div class="bg-surface-card/80 backdrop-blur-md p-4 rounded-xl border border-border-default/60 shadow-brand hover:border-amber-500/40 transition-all duration-300">
        ...
        <div class="bg-surface-card/80 backdrop-blur-md p-4 rounded-xl border border-border-default/60 shadow-brand hover:border-rose-500/40 transition-all duration-300">
    ...
    <div class="bg-surface-card/40 backdrop-blur-md border border-border-default/60 rounded-xl p-4 space-y-4">
        <!-- Search bar -->
        <div class="relative w-full">
            ...
            <input wire:model.debounce.300ms="search" type="text"
                ...
                class="w-full pl-9 pr-4 py-2.5 bg-surface-input border border-border-default/60 rounded-lg text-sm text-text-primary placeholder-text-tertiary focus:border-brand-primary focus:ring-1 focus:ring-brand-primary focus:outline-none transition-all">
        </div>
    ...
    <div wire:loading.class="opacity-60 pointer-events-none" class="transition-opacity duration-200">
        @if($viewMode === 'grid')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                @endphp
                <div class="group bg-surface-card border border-border-default/50 hover:border-brand-primary/30 rounded-xl overflow-hidden shadow-brand flex flex-col justify-between hover:shadow-[0_0_15px_rgba(16,185,129,0.08)] transition-all duration-300 {{ $isDraft ? 'opacity-85' : '' }}">
                    <!-- Photo Header Container -->
                    <div class="relative aspect-[4/3] w-full overflow-hidden bg-surface-raised">
                        <!-- Cover image -->
                        ...
                        <div class="absolute top-3 left-3">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border
                                @switch($listing->status)
                                    ...
                                    @case('sold') bg-zinc-500/10 text-text-secondary border-zinc-500/20 @break
                                    @case('draft') bg-amber-500/10 text-amber-400 border-amber-500/20 @break
                                    @default bg-zinc-500/10 text-text-secondary border-zinc-500/20
                                @endswitch">
                        ...
                        <div class="absolute top-3 right-3 flex items-center gap-1.5 bg-black/60 backdrop-blur-sm px-2 py-1 rounded-full border border-border-strong">
                            @php
                ...
                <span wire:loading wire:target="openListing" class="flex items-center space-x-2 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    ...
                    <div class="p-4 space-y-3 flex-1 flex flex-col justify-between">
                        <div>
                            <!-- Address and suburb -->
                            <h3 class="text-sm font-bold text-text-primary line-clamp-1 group-hover:text-brand-primary transition-colors cursor-pointer" wire:click="openListing({{ $listing->id }})">
                            ...
                            <div class="flex items-center gap-2 mt-2 flex-wrap">
                                ...
                                <span class="inline-flex items-center gap-1 text-[11px] text-text-secondary bg-surface-raised px-2 py-0.5 rounded border border-border-default/40">
                            ...
                            <div class="flex items-center gap-2 text-right">
                                <div>
                                    ...
                                    <span class="text-[9px] px-1.5 py-0.5 rounded-full inline-block mt-1 font-semibold border
                                        ...
                                        @else bg-zinc-500/10 text-text-secondary border-zinc-500/20 @endif">
                                        {{ $dom }} DOM
                                    </span>
                                </div>
                ...
                @empty
                <div class="col-span-full py-16 bg-surface-card/40 border border-dashed border-border-default/60 rounded-xl text-center">
                    <x-ui.empty-state 
                        icon="search" 
        ...
        @elseif($viewMode === 'list')
            <!-- List View Table -->
            <div class="bg-surface-card/80 border border-border-default/60 rounded-xl overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-border-default/60">
                        ...
                        <tbody class="divide-y divide-border-default/30 bg-surface-card/20">
                            @forelse($listings as $listing)
                            @php
                                $dom = $listing->days_on_market ?? ($listing->mandate_start_date ? $listing->mandate_start_date->diffInDays(now()) : 0);
                            @endphp
                            <tr class="hover:bg-surface-sunken/10 transition-colors group cursor-pointer" wire:click="openListing({{ $listing->id }})">
                                ...
                                <td class="px-6 py-3 whitespace-nowrap">
                                    <div class="h-12 w-16 bg-surface-raised rounded-lg overflow-hidden border border-border-default/45">
                                        @if($listing->coverPhoto)
                                ...
                                <!-- Address -->
                                <td class="px-6 py-3">
                                    <p class="text-sm font-bold text-text-primary group-hover:text-brand-primary transition-colors truncate max-w-xs">{{ $listing->property->address_line_1 }}</p>
        ...
        @elseif($viewMode === 'map')
            <!-- Map View Layout -->
            <div class="flex flex-col lg:flex-row bg-surface-card border border-border-default/60 rounded-xl overflow-hidden h-[550px]">
                ...
                <div class="flex-1 relative bg-[#02050b] overflow-hidden" x-data="{ activePin: null }">
                    ...
                    @foreach($listings as $listing)
                        ...
                        <div class="absolute" style="top: {{ $top }}%; left: {{ $left }}%;"
                            ...
                            <div class="relative cursor-pointer group/pin">
                                ...
                                <!-- Pin container -->
                                <div class="relative flex items-center gap-1.5 bg-surface-page border-2 border-brand-primary hover:border-white px-2.5 py-1 rounded-full shadow-[0_0_12px_rgba(16,185,129,0.3)] transition-all">
                                    <div class="h-2 w-2 rounded-full bg-brand-primary"></div>
                                    <span class="text-[10px] font-bold font-mono text-text-primary">{{ $priceText }}</span>
                                </div>
                                ...
                                <div x-show="activePin === {{ $listing->id }}"
                                     ...
                                     class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 bg-surface-card border border-border-default rounded-xl p-3 shadow-2xl z-20 pointer-events-auto">
                                    <div class="relative rounded-lg overflow-hidden h-28 bg-surface-raised mb-2">
                                        @if($listing->coverPhoto)
                                        <img src="{{ asset('storage/' . $listing->coverPhoto->file_path) }}" class="h-full w-full object-cover">
                                        @else
                                        <div class="h-full w-full flex items-center justify-center text-text-tertiary">
                                    ...
                                    <h4 class="text-xs font-bold text-text-primary truncate">{{ $listing->property->address_line_1 }}</h4>
                    ...
                    <div class="absolute bottom-4 left-4 flex flex-col gap-1 bg-surface-card/80 backdrop-blur-sm border border-border-default/50 p-1.5 rounded-lg">
                        <button class="w-7 h-7 text-xs font-bold text-text-secondary hover:text-text-primary bg-surface-raised rounded flex items-center justify-center">+</button>
                        <button class="w-7 h-7 text-xs font-bold text-text-secondary hover:text-text-primary bg-surface-raised rounded flex items-center justify-center">-</button>
                    </div>
                </div>

                <!-- Right Viewport Sidebar (320px) -->
                <div class="w-full lg:w-[320px] bg-surface-card border-l border-border-default/50 flex flex-col h-full">
                    <div class="p-4 border-b border-border-default/50">
                        <h3 class="text-sm font-bold text-text-primary">Viewport Results</h3>
                    ...
                    <div class="flex-1 overflow-y-auto p-4 space-y-3">
                        ...
                        <div wire:click="openListing({{ $listing->id }})" class="flex items-center gap-3 p-2 rounded-lg bg-surface-raised/40 hover:bg-surface-raised border border-border-default/30 transition-all cursor-pointer group">
                            <div class="h-12 w-12 rounded overflow-hidden bg-surface-raised shrink-0">
                                @if($listing->coverPhoto)
                                <img src="{{ asset('storage/' . $listing->coverPhoto->file_path) }}" class="h-full w-full object-cover">
                                @else
                                <div class="h-full w-full flex items-center justify-center text-text-tertiary">
                            ...
                            <div class="min-w-0">
                                <h4 class="text-xs font-bold text-text-primary group-hover:text-brand-primary truncate">{{ $listing->property->address_line_1 }}</h4>
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div>
                                            ...
                                            <select wire:model.defer="property_type"
                                                class="w-full rounded-md border border-border-default/60 bg-surface-input px-3 py-2.5 text-sm text-text-primary focus:border-brand-primary focus:outline-none transition-all">

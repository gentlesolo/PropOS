    ...
    <div class="flex items-center justify-between border-b border-border-default pb-4">
        ...
        <div class="flex items-center bg-surface-card border border-border-default p-0.5 rounded">
            <button wire:click="setViewMode('agent')" 
                    class="disabled:opacity-70 disabled:cursor-not-allowed relative px-2.5 py-1 rounded text-[10px] font-black uppercase tracking-wider transition-all duration-200 {{ $viewMode === 'agent' ? 'bg-brand-primary text-[#030712]' : 'text-text-secondary hover:text-text-primary' }}" wire:loading.attr="disabled" wire:target="setViewMode">
                <span wire:loading.remove wire:target="setViewMode">Agent Terminal</span>
            ...
            <button wire:click="setViewMode('principal')" 
                    class="disabled:opacity-70 disabled:cursor-not-allowed relative px-2.5 py-1 rounded text-[10px] font-black uppercase tracking-wider transition-all duration-200 {{ $viewMode === 'principal' ? 'bg-brand-primary text-[#030712]' : 'text-text-secondary hover:text-text-primary' }}" wire:loading.attr="disabled" wire:target="setViewMode">
                <span wire:loading.remove wire:target="setViewMode">Principal Room</span>
                <span wire:loading wire:target="setViewMode" class="flex items-center space-x-2 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    <svg class="animate-spin h-4 w-4 text-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                </span>
            </button>

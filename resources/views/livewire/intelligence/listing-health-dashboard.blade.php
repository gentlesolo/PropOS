<div>
    ...
    <div class="bg-surface-card rounded-2xl border border-border-default overflow-hidden shadow-sm">
        <div class="px-5 py-4 border-b border-border-default flex items-center justify-between bg-surface-sunken/30 gap-4">
            ...
            <div class="flex gap-2">
                ...
                <button wire:click="$set('filterHealth', '{{ $val }}')"
                    class="disabled:opacity-70 disabled:cursor-not-allowed relative px-3 py-1.5 rounded-lg text-xs font-bold transition-colors
                    {{ $filterHealth === $val ? 'bg-gradient-to-br from-brand-primary to-brand-primary/80 text-white shadow-brand-sm ring-1 ring-white/10' : 'border border-border-default bg-surface-card text-text-secondary hover:bg-surface-sunken' }}" wire:loading.attr="disabled" wire:target="$set">

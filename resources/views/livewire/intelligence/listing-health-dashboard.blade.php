        ...
        <div class="px-5 py-4 border-b border-border-default flex items-center justify-between bg-surface-sunken/30 gap-4">
            ...
            <div class="flex gap-2">
                ...
                <button wire:click="$set('filterHealth', '{{ $val }}')"
                    ...
                    {{ $filterHealth === $val ? 'bg-gradient-to-br from-brand-primary to-brand-primary/80 text-white shadow-brand-sm ring-1 ring-white/10' : 'border border-border-default bg-surface-card text-text-secondary hover:bg-surface-sunken' }}">
                    {{ $label }}
                </button>
                @endforeach
            </div>

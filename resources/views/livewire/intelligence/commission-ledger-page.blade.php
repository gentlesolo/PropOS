<div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        <div class="glass-panel rounded-2xl border border-border-default/60 p-5 md:col-span-1">
            ...
            <h3 class="text-2xl font-black text-text-primary">₦{{ number_format($stats['gross_ytd'] / 1000000, 1) }}M</h3>
        </div>
        <div class="glass-panel rounded-2xl border border-border-default/60 p-5">
            ...
            <h3 class="text-2xl font-black text-brand-primary">₦{{ number_format($stats['agency_ytd'] / 1000000, 1) }}M</h3>
        </div>
        <div class="glass-panel rounded-2xl border border-border-default/60 p-5">
            <p class="text-[10px] font-bold text-text-tertiary uppercase tracking-wider mb-1">Agents Net</p>
            <h3 class="text-2xl font-black text-success-600">₦{{ number_format($stats['agents_ytd'] / 1000000, 1) }}M</h3>
        </div>
        <div class="glass-panel rounded-2xl border border-border-default/60 p-5">
            <p class="text-[10px] font-bold text-text-tertiary uppercase tracking-wider mb-1">Paid Out</p>
            <h3 class="text-2xl font-black text-text-primary">₦{{ number_format($stats['paid'] / 1000000, 1) }}M</h3>
        </div>
        <div class="glass-panel rounded-2xl border border-border-default/60 p-5">
            <p class="text-[10px] font-bold text-text-tertiary uppercase tracking-wider mb-1">Pending Payouts</p>
            <h3 class="text-2xl font-black text-warning-600">{{ $stats['pending_count'] }}</h3>
        </div>
    </div>
                    ...
                    <tr class="hover:bg-surface-raised/20 transition-colors">
                        ...
                        <td class="py-4 px-5">
                            <div class="flex items-center gap-2">
                                ...
                                <a href="{{ route('compliance.transaction.detail', $commission->transaction) }}" class="text-xs text-brand-primary hover:text-brand-secondary font-medium">TXN →</a>
                                @endif
                            </div>
                        </td>
                    </tr>

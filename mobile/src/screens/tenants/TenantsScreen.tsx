  ...
  return (
    <Pressable
      ...
      <View className="flex-1">
        <Text className="text-white font-medium">{tenant.full_name ?? 'Unknown'}</Text>
        <Text className="text-slate-400 text-xs mt-0.5" numberOfLines={1}>
          {tenant.property ?? 'No property linked'}
        </Text>
      ...
      <View className="items-end gap-1">
        ...
        {tenant.fica_count === 0 && (
          <Text className="text-amber-500 text-xs">FICA pending</Text>
        )}
      </View>
    </Pressable>
  );
  ...
  return (
    <View className="flex-1 bg-surface-base">
      ...
      <View className="pt-14 pb-4 px-4 bg-surface-card border-b border-slate-800">
        <Text className="text-2xl font-bold text-white mb-3">Tenants</Text>
        <TextInput
          value={search}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="flex-1 items-center justify-center py-16">
              <Text className="text-slate-500 text-sm">No tenants found.</Text>
            </View>
          }
        />
      )}
